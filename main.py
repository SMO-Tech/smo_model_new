...
class CompleteSoccerAnalysisPipeline:
    ...
    def analyze_video(self, video_path: str, frame_count: int = -1, output_suffix: str = "_complete_analysis"):
        ...
        for i, frame in enumerate(tqdm(frames, desc="Processing frames")):
            ...
            if pitch_calibration.enabled and len(player_detections.xyxy) > 0:
                ...
                player_pitch_coords = [None] * len(player_detections.xyxy)
            player_detections.pitch_xy = player_pitch_coords

            # Record pitch coordinates for ball
    ...
    def _write_pitch_debug_csv(self, rows: List[dict], video_path: str):
        """Write debug CSV with pitch coordinates."""
        if not rows:
            return
        ...
        csv_path = debug_dir / "pitch_coordinates.csv"
        df.to_csv(csv_path, index=False)
        print(f"✅ Created pitch debug CSV: {csv_path}")
